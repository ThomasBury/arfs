...
class VariableImportance(SelectorMixin, BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        support_ordered = self.feature_importances_summary_["cumulative_importance"] >= self.threshold
        to_drop = list(self.feature_importances_summary_.loc[support_ordered, "feature"])
    
        self.support_ = np.asarray([False if c in to_drop else True for c in self.feature_names_in_])
    ...
    def plot_importance(self, figsize=None, plot_n=50, n_feat_per_inch=3, log=True, style=None):
        ...
        importance_index = np.min(np.where(df["cumulative_importance"] > self.threshold))
        ...
        x_vert, y_vert = [importance_index, importance_index], [importance_min_value_on_axis, self.threshold]
        x_horiz, y_horiz = [importance_min_value_on_axis, importance_index], [self.threshold, self.threshold]
...
def _compute_varimp_lgb(X,
    ...
    if encode:
        encoder = OrdinalEncoderPandas(**encoder_kwargs) if encoder_kwargs is not None else OrdinalEncoderPandas()
